...
func main() {
    orders := []string{"cafelatte", "americanoice", "hotcafelatte", "anything"}
    fmt.Println(solution(orders))
}


func solution(orders []string) int {
    total := 0
    menus := []string{"americano", "latte", "anything"}
    for _, order := range orders {
        for _, menu := range menus {
            if strings.Contains(order, menu) {
                if menu == "americano" {
                    total += 4500
                } else if menu == "latte" {
                    total += 5000
